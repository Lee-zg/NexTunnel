...
v0.6.5-alpha 的生产可用核心是自部署 Relay/Dashboard、桌面 TCP/HTTP 隧道、客户端监控、服务端安装脚本和诊断闭环。P2P、TUN、eBPF 和 Edge 能力按验证工具链与当前平台条件明确边界。
...
v0.6.5-alpha 中 macOS P2P/Relay 可用。系统路由 TUN 需要安装官方内置 helper：可在网络页点击“安装 Helper”并输入管理员密码，或执行 `sudo nextunnel helper install`。helper 会安装到 `/Library/PrivilegedHelperTools/nextunnel-helper` 并加载 `com.nextunnel.helper` LaunchDaemon。没有 helper 验证报告时，只能标注为外部阻塞或预览限制。
