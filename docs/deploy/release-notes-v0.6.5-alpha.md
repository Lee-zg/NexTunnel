...
Windows 安装器现在会正确解码本地化命令输出，避免 `taskkill.exe` 或 PowerShell 返回 GB18030/UTF-16 文本时在界面中显示乱码；同时把“没有找到旧进程”视为可继续状态，避免无旧版本运行时误判为安装失败。快捷方式创建已改为直接调用 Windows COM API，不再把路径追加到 PowerShell `-Command`，从根源修复带空格路径触发的 `UnexpectedToken` 安装失败。

## Capability Status

| 能力 | 状态 | 说明 |
| --- | --- | --- |
| Windows 自定义安装器 | 已修复 | 本地化命令输出统一解码为 UTF-8；快捷方式通过 COM API 创建，不再受 PowerShell 参数解析影响。 |
...
v0.6.5-alpha 可以作为公开 alpha 重新发布，修复 Windows 安装器中文乱码、快捷方式创建失败和升级误判问题，并发布最新 macOS TUN 处理结果。没有新的 `tun-macos-latest.json`、Dashboard HTTPS、eBPF 压测和真实多地域拓扑报告前，仍不把对应能力描述为生产通过。
